...
6. Update both `catalog.json` and `sitemap.xml` for every public tool.
...
- Add a public tool without a catalog entry and sitemap entry.
...
2. Create only an isolated proposed tool folder and provide separate integration snippets; or
3. Stop and explain which unknown project contracts prevent a safe change.

The AI MUST NOT blindly overwrite `catalog.json`, `sitemap.xml`, shared assets, routing files, or deployment configuration. It MUST NOT claim the tool is integrated or tested.

## Project architecture

NikTool is currently a static site with no required build step.

```text
.
|-- index.html                 Homepage and catalog shell
|-- catalog.json               Homepage tool cards, filters, and search data
|-- sitemap.xml                Search-engine discovery list
...
| Homepage card and search | `catalog.json` |
...
`catalog.json` is not a substitute for HTML metadata. Search engines index each tool's `index.html`.
...
Add one object to the root `catalog.json` array:
...
- Keep the object formatting consistent with neighboring entries

## Sitemap contract

Add one entry before `</urlset>`:
...
### 4. Validate catalog uniqueness and required fields
...
### 5. Validate catalog folders and pages
...
### 7. Search for unfinished placeholders
...
```powershell
git status --short
git diff -- catalog.json sitemap.xml tools/<slug>/ README.md
```
...
- [ ] Catalog entry exists exactly once
- [ ] Sitemap entry exists exactly once
...
For stronger scaling guarantees, the next engineering step should be automated enforcement:

1. A JSON Schema for `catalog.json`
2. A `scripts/validate-tools.cjs` validator
3. A canonical tool scaffolding script
4. Automated sitemap/catalog parity checks
5. HTML metadata checks
6. Browser smoke tests
7. CI that blocks invalid tool changes

Until those checks exist, every AI agent MUST execute the validation commands and definition-of-done checklist in this README.
