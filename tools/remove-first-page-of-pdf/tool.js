(() => {
  ...
  fileInput.addEventListener('change', () => {
    ...
    if (fileInput.files && fileInput.files[0]) {
      ...
      setMessage('Select a PDF file with at least 2 pages to begin.');
    }
    updateControls();
  });
  ...
  primaryButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (totalPages < 2) {
        setMessage(`This PDF has only 1 page. You need a document with at least 2 pages to remove the first page.`, 'error');
        return;
      }
      ...
      const outputFilename = file.name.replace(/\.pdf$/i, '') + '-no-first-page.pdf';
      downloadButton.href = objectUrl;
      downloadButton.download = outputFilename;
      downloadButton.style.display = 'inline-block';

      setMessage(`Success! Removed page 1. New page count: ${totalPages - 1}. Click Download below.`, 'success');
    } catch (err) {
